(function () {
  ...
  async function init() {
    ...
    function bumpRec(lastText){
      ...
      saveRec(r);
      const cntEl = document.getElementById("myRouletteCnt");
      const lastEl = document.getElementById("myLast");
      if (cntEl) cntEl.textContent = (r.rouletteCnt || 0) + " 次";
      if (lastEl) lastEl.textContent = r.last || "—";
    }
    // render once
    (function renderRec(){
      const r = loadRec();
  ...
  // 給 SPA 用：進入 game 時確保初始化
  window.Roulette = {
    ensureInit: () => init()
  };

  // 如果你直接開在 game，也能自動 init
  document.addEventListener("DOMContentLoaded", () => {
    const hash = location.hash || "#/home";
    if (hash.includes("game")) init();
  });
})();
